...
public class Lab6 extends GLJPanel implements GLEventListener  {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        //
        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT, materials[8], 0);
        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, materials[8], 4 );
        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, materials[8], 8 );
        gl2.glMaterialf( GL2.GL_FRONT_AND_BACK, GL2.GL_SHININESS, materials[8][12] );

        final int verticles = 17; //
        final int radius = 6;
        final int height = 6;

        final float baseOffsetY = -2.0f;
        ...
        for (int i = 1; i <= verticles; i++) {
            ...
            x0 = (float) (radius * Math.cos(angle));
            z0 = (float) (radius * Math.sin(angle));

            angle = i * increment;

            x1 = (float) (radius * Math.cos(angle));
            z1 =(float) (radius * Math.sin(angle));

            gl2.glNormal3f(
                    (baseOffsetY - height) * (z0 - z1),
                    z0 * (x0 - x1) - x0 * (z0 - z1),
                    - (baseOffsetY - height) * (x0 - x1)
            );

            gl2.glVertex3f(x0, baseOffsetY, z0);
            gl2.glVertex3f(0, height, 0 );
            gl2.glVertex3f(x1, baseOffsetY, z1);
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        float[] diffuseColor  = mapRGBcolor(56, 122, 250);
        float[] specularColor  = mapRGBcolor(48, 181, 175);
        float[] ambientColor  = mapRGBcolor(225, 132, 86);

        float[] ligtPosition = {1,0,0,0};
        ...
        gl2.glLightfv (gl2.GL_LIGHT1, GLLightingFunc.GL_POSITION, ligtPosition, 0);

        gl2.glEnable(GL2.GL_LIGHT1);
    }

    public float[] mapRGBcolor(float r, float g, float b) {

        float r1 = mapValue(r, 0, 255, 0, 1.0f);
        float g1 = mapValue(g, 0, 255, 0, 1.0f);
        float b1 = mapValue(b, 0, 255, 0, 1.0f);

        float[] output = {r1, g1, b1, 1};

        return output;
    }

    public float mapValue(float input, float input_start, float input_end, float output_start, float output_end) {

        float output = 0;

        double slope = 1.0 * (output_end - output_start) / (input_end - input_start);
        output = (float) (output_start + slope * (input - input_start));
